...
'    If UserList(UserIndex).Stats.ELV < MINPARTYLEVEL Then
    ...
    If CInt(UserList(UserIndex).Stats.UserAtributos(eAtributos.Carisma)) * UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 100 Then
        Call WriteConsoleMsg(1, UserIndex, "Tu carisma y liderazgo no son suficientes para liderar una party.", FontTypeNames.FONTTYPE_PARTY)
...
If UserList(UserIndex).PartyIndex = 0 Then
    If UserList(UserIndex).flags.Muerto = 0 Then
        If UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) >= 5 Then
            ...
            If tInt = -1 Then
                Call WriteConsoleMsg(1, UserIndex, "Por el momento no se pueden crear mas parties", FontTypeNames.FONTTYPE_PARTY)
                Exit Sub
            Else
                Set Parties(tInt) = New clsParty
                If Not Parties(tInt).NuevoMiembro(UserIndex) Then
                    Call WriteConsoleMsg(1, UserIndex, "La party está llena, no puedes entrar", FontTypeNames.FONTTYPE_PARTY)
                    Set Parties(tInt) = Nothing
                    Exit Sub
                Else
                    Call WriteConsoleMsg(1, UserIndex, "¡Has formado una party!", FontTypeNames.FONTTYPE_PARTY)
                    UserList(UserIndex).PartyIndex = tInt
                    UserList(UserIndex).PartySolicitud = 0
                    If Not Parties(tInt).HacerLeader(UserIndex) Then
                        Call WriteConsoleMsg(1, UserIndex, "No puedes hacerte líder.", FontTypeNames.FONTTYPE_PARTY)
                    Else
                        Call WriteConsoleMsg(1, UserIndex, "¡ Te has convertido en líder de la party !", FontTypeNames.FONTTYPE_PARTY)
                    End If
                End If
            End If
        Else
            Call WriteConsoleMsg(1, UserIndex, " No tienes suficientes puntos de liderazgo para liderar una party.", FontTypeNames.FONTTYPE_PARTY)
        End If
    Else
        Call WriteConsoleMsg(1, UserIndex, "Estás muerto!", FontTypeNames.FONTTYPE_PARTY)
    End If
Else
    Call WriteConsoleMsg(1, UserIndex, " Ya perteneces a una party.", FontTypeNames.FONTTYPE_PARTY)
End If
End Sub
...
Dim tInt As Integer
    ...
    If UserList(UserIndex).PartyIndex > 0 Then
        ...
        Call WriteConsoleMsg(1, UserIndex, " Ya perteneces a una party, escribe /SALIRPARTY para abandonarla", FontTypeNames.FONTTYPE_PARTY)
        UserList(UserIndex).PartySolicitud = 0
        Exit Sub
    End If
    ...
    If tInt > 0 Then
        If UserList(tInt).PartyIndex > 0 Then
            ...
            Call WriteConsoleMsg(1, UserIndex, " El fundador decidirá si te acepta en la party", FontTypeNames.FONTTYPE_PARTY)
        Else
            Call WriteConsoleMsg(1, UserIndex, UserList(tInt).name & " no es fundador de ninguna party.", FontTypeNames.FONTTYPE_INFO)
            UserList(UserIndex).PartySolicitud = 0
            Exit Sub
        End If
    Else
        Call WriteConsoleMsg(1, UserIndex, " Para ingresar a una party debes hacer click sobre el fundador y luego escribir /PARTY", FontTypeNames.FONTTYPE_PARTY)
        UserList(UserIndex).PartySolicitud = 0
    End If
End Sub
...
If PI > 0 Then
    ...
    End If
Else
    Call WriteConsoleMsg(1, UserIndex, " No eres miembro de ninguna party.", FontTypeNames.FONTTYPE_INFO)
End If
...
If PI = UserList(OldMember).PartyIndex Then
    ...
    End If
Else
    Call WriteConsoleMsg(1, leader, LCase(UserList(OldMember).name) & " no pertenece a tu party.", FontTypeNames.FONTTYPE_INFO)
End If
...
'*************************************************
    ...
    If PI > 0 Then
        ...
        Else
            Call WriteConsoleMsg(1, User, "¡No eres el líder de tu Party!", FontTypeNames.FONTTYPE_PARTY)
            Exit Function
        End If
    Else
        Call WriteConsoleMsg(1, User, "No eres miembro de ninguna party.", FontTypeNames.FONTTYPE_INFO)
        Exit Function
    End If
End Function
        ...
        If UserList(NewMember).PartyIndex = 0 Then
            If Parties(PI).PuedeEntrar(NewMember, razon) Then
                ...
                Else
                    ...
                    Call SendData(SendTarget.ToAdmins, leader, PrepareMessageConsoleMsg(1, " Servidor> CATASTROFE EN PARTIES, NUEVOMIEMBRO DIO FALSE! :S ", FontTypeNames.FONTTYPE_PARTY))
                    End If
                Else
                'no debe entrar
                Call WriteConsoleMsg(1, leader, razon, FontTypeNames.FONTTYPE_PARTY)
            End If
        Else
            Call WriteConsoleMsg(1, leader, UserList(NewMember).name & " ya es miembro de otra party.", FontTypeNames.FONTTYPE_PARTY)
            Exit Sub
        End If
    Else
        Call WriteConsoleMsg(1, leader, "¡Está muerto, no puedes aceptar miembros en ese estado!", FontTypeNames.FONTTYPE_PARTY)
        Exit Sub
    End If
Else
    Call WriteConsoleMsg(1, leader, LCase(UserList(NewMember).name) & " no ha solicitado ingresar a tu party.", FontTypeNames.FONTTYPE_PARTY)
    Exit Sub
End If
...
Dim texto As String
    ...
    If PI > 0 Then
        ...
        Call WriteConsoleMsg(1, UserIndex, texto, FontTypeNames.FONTTYPE_PARTY)
    End If
...
If PI = UserList(NewLeader).PartyIndex Then
    If UserList(NewLeader).flags.Muerto = 0 Then
        ...
        Else
            Call WriteConsoleMsg(1, OldLeader, "¡No se ha hecho el cambio de mando!", FontTypeNames.FONTTYPE_PARTY)
        End If
    Else
        Call WriteConsoleMsg(1, OldLeader, "¡Está muerto!", FontTypeNames.FONTTYPE_INFO)
    End If
Else
    Call WriteConsoleMsg(1, OldLeader, LCase(UserList(NewLeader).name) & " no pertenece a tu party.", FontTypeNames.FONTTYPE_INFO)
End If
...
If Not PARTY_EXPERIENCIAPORGOLPE Then
    ...
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(1, "Servidor> Distribuyendo experiencia en parties.", FontTypeNames.FONTTYPE_SERVER))
    For i = 1 To MAX_PARTIES
        If Not Parties(i) Is Nothing Then
            Call Parties(i).FlushExperiencia
        End If
    Next i
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(1, "Servidor> Experiencia distribuida.", FontTypeNames.FONTTYPE_SERVER))
    Call SendData(SendTarget.ToAll, 0, PrepareMessagePauseToggle())
    haciendoBK = False
