...
If UserList(UserIndex).Stats.UserAtributos(eAtributos.Fuerza) = 0 Then
    Call WriteErrorMsg(UserIndex, "Atributos inválidos.")
    Exit Sub
End If
...
UserList(UserIndex).Stats.GLD = 200000
...
UserList(UserIndex).Invent.Object(1).ObjIndex = 854
UserList(UserIndex).Invent.Object(1).amount = 1

UserList(UserIndex).Invent.Object(2).ObjIndex = 879
...
Select Case UserRaza
    ...
    Case eRaza.Enano
        UserList(UserIndex).Invent.Object(4).ObjIndex = 240
    Case eRaza.Gnomo
        UserList(UserIndex).Invent.Object(4).ObjIndex = 240
    Case eRaza.Orco
        UserList(UserIndex).Invent.Object(4).ObjIndex = 464
...
Call SaveUser(UserIndex, CharPath & UCase$(name) & ".chr", Account, name)
...
Sub CloseSocket(ByVal UserIndex As Integer, Optional socket As Boolean = True)
...
On Error GoTo Errhandler
    ...
    If UserList(UserIndex).ConnID <> -1 And socket = True Then
        Call CloseSocketSL(UserIndex)
    End If
...
If UserList(UserIndex).Pos.map = 0 Then
    Select Case UserList(UserIndex).Hogar
        Case eCiudad.cNix
            UserList(UserIndex).Pos = Nix 'Nix
    End Select
Else
...
Call WriteChangeMap(UserIndex, UserList(UserIndex).Pos.map, MapInfo(UserList(UserIndex).Pos.map).MapVersion, MapInfo(UserList(UserIndex).Pos.map).name) 'Carga el mapa
...
If UserList(UserIndex).flags.Privilegios = PlayerType.Dios Then
    UserList(UserIndex).flags.ChatColor = RGB(232, 225, 0)
...
If NumUsers > recordusuarios Then
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(1, "Record de usuarios conectados simultaneamente." & "Hay " & NumUsers & " usuarios.", FontTypeNames.FONTTYPE_INFO))
