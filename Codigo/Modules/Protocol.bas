...
'***************************************************
    ...
    With FontTypes(FontTypeNames.FONTTYPE_CAOS)
        .red = 122
        .bold = 1
...
On Error Resume Next
    Debug.Print incomingData.PeekByte
    Select Case incomingData.PeekByte()
        ...
        Case ServerPacketID.ShowCorreo              ' CORRE
            Call HandleShowCorreo
...
'***************************************************
    ...
    'Reset global vars
    IScombate = False: frmMain.modocombate.Visible = False: frmMain.nomodocombate.Visible = True
    UserDescansar = False
...
'***************************************************
    ...
    'Call AddtoRichTextBox(frmMain.RecChat, MENSAJE_SEGURO_RESU_OFF, 255, 0, 0, True, False, False)
End Sub
...
'***************************************************
    ...
    'Call AddtoRichTextBox(frmMain.RecChat, MENSAJE_SEGURO_RESU_ON, 0, 255, 0, True, False, False)
End Sub
...
'***************************************************
    ...
    If frmMain.InMouseExp Then
        frmMain.lblExp.Caption = UserExp & "/" & UserPasarNivel
    Else
        If Not UserExp = 0 And Not UserPasarNivel = 0 Then frmMain.lblExp.Caption = str(Round(CDbl(UserExp) * CDbl(100) / CDbl(UserPasarNivel), 2) & "%") Else frmMain.lblExp.Caption = "¡Nivel maximo!"
    End If
...
'For now we just drop it
    ...
    frmMain.lblMapName.Caption = incomingData.ReadASCIIString()
...
On Error GoTo ErrHandler
    ...
    name = Left$(charlist(CharIndex).nombre, Pos - 2)
    If charlist(CharIndex).nombre <> "" Then _
        Call AddtoRichTextBox(frmMain.RecChat, "[" & name & "]" & chat, r, g, b, False, False)
    If name <> "" And Not (r = 128 And g = 128 And b = 0) And chat <> "'Falla'" And AttactMsg(chat, RGB(r, g, b)) Then _
        Call AddtoRichTextBox(frmMain.RecChat, name & ">" & chat, r, g, b, True, False)
...
On Error GoTo ErrHandler
    ...
    With charlist(CharIndex)
        Call Engine.SetCharacterFx(CharIndex, Buffer.ReadInteger(), Buffer.ReadInteger())
...
Sub HandleCharStatus()
    ...
    Select Case Priv
        ...
        Case 2 'Azul
            Engine.Long_To_RGB_List charlist(CharIndex).label_color, D3DColorXRGB(0, 0, 255)
        Case 3 'Rojo
            Engine.Long_To_RGB_List charlist(CharIndex).label_color, D3DColorXRGB(255, 0, 0)
        Case 4 'Naranja
        ...
        Case 6 'Armada Real
            Engine.Long_To_RGB_List charlist(CharIndex).label_color, D3DColorXRGB(190, 220, 0)
    End Select
...
'***************************************************
    ...
    With charlist(CharIndex)
        ...
        If .Muerto = False And oldMuerto = True Then
            Call Engine.Char_Particle_Group_Remove(CharIndex, 22)
        End If
        ...
        PonerAura CharIndex, Escudo, Arma, body
        
        tempInt = incomingData.ReadInteger()
        If tempInt <> 0 Then .Casco = CascoAnimData(tempInt)
        
        Call Engine.SetCharacterFx(CharIndex, incomingData.ReadInteger(), incomingData.ReadInteger())
    End With
...
'***************************************************
    ...
    If Obj = 669 Then
        Call Engine.General_Particle_Create(34, x, y)
        Engine.Light_Create x, y, D3DColorXRGB(255, 126, 65), 2
        MapData(x, y).OBJInfo.name = incomingData.ReadASCIIString
    Else
        MapData(x, y).ObjGrh.grhindex = Obj
        MapData(x, y).OBJInfo.name = incomingData.ReadASCIIString
    End If
...
'***************************************************
    ...
    If MapData(x, y).particle_group_index > 0 Then
        If Engine.Particle_Get_Type(MapData(x, y).particle_group_index) = 16 Then
...
'***************************************************
    ...
    bTecho = (MapData(UserPos.x, UserPos.y).Trigger = 1 Or _
            MapData(UserPos.x, UserPos.y).Trigger = 2 Or _
            MapData(UserPos.x, UserPos.y).Trigger = 4 Or _
            MapData(UserPos.x, UserPos.y).Trigger >= 20)
    If bRain Then
        ...
        End If
        frmMain.IsPlaying = PlayLoop.plnone
        Engine.Particle_Group_Remove meteo_particle
        meteo_particle = 0
    Else
        meteo_particle = Engine.General_Particle_Create(56, -1, -1)
    End If
...
'***************************************************
    ...
    Call Engine.SetCharacterFx(CharIndex, fX, Loops)
End Sub
...
'***************************************************
    ...
    Select Case UsingSkill
        Case Magia
            Call AddtoRichTextBox(frmMain.RecChat, MENSAJE_TRABAJO_MAGIA, 100, 100, 120, 0, 0)
        Case Pesca
            Call AddtoRichTextBox(frmMain.RecChat, MENSAJE_TRABAJO_PESCA, 100, 100, 120, 0, 0)
        Case Robar
            Call AddtoRichTextBox(frmMain.RecChat, MENSAJE_TRABAJO_ROBAR, 100, 100, 120, 0, 0)
        Case Talar
            Call AddtoRichTextBox(frmMain.RecChat, MENSAJE_TRABAJO_TALAR, 100, 100, 120, 0, 0)
        Case Mineria
            Call AddtoRichTextBox(frmMain.RecChat, MENSAJE_TRABAJO_MINERIA, 100, 100, 120, 0, 0)
        Case FundirMetal
            Call AddtoRichTextBox(frmMain.RecChat, MENSAJE_TRABAJO_FUNDIRMETAL, 100, 100, 120, 0, 0)
        Case Proyectiles
            Call AddtoRichTextBox(frmMain.RecChat, MENSAJE_TRABAJO_PROYECTILES, 100, 100, 120, 0, 0)
    End Select
End Sub
...
'***************************************************
    ...
    End If
End Sub
' Handles the ShowCorreo message.

Private Sub HandleShowCorreo()
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'
...
'***************************************************
    ...
    Call AddtoRichTextBox(frmMain.RecChat, "El ping es " & (GetTickCount - pingTime) & " ms.", 255, 0, 0, True, False, False)
...
Public Sub WriteRainToggle(Optional ByVal trueno As Byte = 0)
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Writes the "RainToggle" message to the outgoing data buffer
'***************************************************
    ...
    Call outgoingData.WriteByte(trueno)
End Sub
...
Private Sub HandleParticle()
    ...
    Life = incomingData.ReadInteger
    
    If Not InMapBounds(x, y) And Not Particula < 1 And Not Particula > TotalStreams Then
        Call Engine.General_Particle_Create(Particula, x, y, Life)
    End If
End Sub
Private Sub HandleCharParticle()
    ...
    If Not Particula < 1 And Not Particula > TotalStreams Then
        Call Engine.General_Char_Particle_Create(Particula, CharIndex)
    End If
End Sub
...
Private Sub HandleDestCharParticle()
    ...
    If Not Particula < 1 And Not Particula > TotalStreams Then
        Call Engine.Char_Particle_Group_Remove(CharIndex, Particula)
    End If
End Sub
Public Sub WriteSinDuelo()
    Call outgoingData.WriteByte(ClientPacketID.SinDuelo)
End Sub
Public Sub WriteSinDeath()
    Call outgoingData.WriteByte(ClientPacketID.SinDeath)
End Sub
Public Sub WriteDuelo()
    Call outgoingData.WriteByte(ClientPacketID.Duelo)
End Sub
Public Sub WriteDeath()
    Call outgoingData.WriteByte(ClientPacketID.Death)
End Sub
Public Sub HandleAddPj()
    ...
    Dim Casco As Byte, Arma As Byte, Escudo As Byte, Nivel As Byte, Clase As Byte, Mapa As Integer, Index As Byte, color As Byte
...
Public Sub WriteLoginExistingChar()
    With outgoingData
        ...
        UserName = cPJ(frmPanelAccount.Seleccionado).nombre
...
Public Sub WriteLoginNewAccount()
    With outgoingData
        ...
        Call .WriteASCIIString(UserAnswer)
        
        Call .WriteByte(UserQuestion)
...
Public Sub WritebBeta(ByVal arg As Byte)
    With outgoingData
        Call .WriteByte(ClientPacketID.Beta)
        Call .WriteByte(arg)
    End With
End Sub
