...
If res <= Suerte Then
    ...
    Call WriteConsoleMsg(1, UserIndex, "¡Te has escondido entre las sombras!", FontTypeNames.FONTTYPE_INFO)
    Call SubirSkill(UserIndex, Ocultarse)
Else
    '[CDT 17-02-2004]
    If Not UserList(UserIndex).flags.UltimoMensaje = 4 Then
        Call WriteConsoleMsg(1, UserIndex, "¡No has logrado esconderte!", FontTypeNames.FONTTYPE_INFO)
...
If UserList(UserIndex).Stats.UserSkills(Equitacion) < ObjData(Obj).MinSkill Then
    Call WriteConsoleMsg(1, UserIndex, "Para usar esta montura necesitas " & ObjData(Obj).MinSkill & " puntos en equitación.", FontTypeNames.FONTTYPE_INFO)
    Exit Function
End If
...
If UserList(UserIndex).flags.TargetObjInvIndex > 0 Then
   ...
   Else
        Call WriteConsoleMsg(1, UserIndex, "No tenes conocimientos de mineria suficientes para trabajar este mineral.", FontTypeNames.FONTTYPE_INFO)
   End If
...
If MapInfo(UserList(UserIndex).Pos.Map).Pk = False Then
   Call WriteConsoleMsg(1, UserIndex, "No esta permitido Pescar en zonas seguras.", FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
...
If res <= 6 Then
    ...
    Call WriteConsoleMsg(1, UserIndex, "¡Has pescado un lindo pez!", FontTypeNames.FONTTYPE_INFO)
    
Else
    '[CDT 17-02-2004]
    If Not UserList(UserIndex).flags.UltimoMensaje = 6 Then
      Call WriteConsoleMsg(1, UserIndex, "¡No has pescado nada!", FontTypeNames.FONTTYPE_INFO)
...
If Suerte > 0 Then
    ...
    If res < 6 Then
        ...
        Call WriteConsoleMsg(1, UserIndex, "¡Has pescado algunos peces!", FontTypeNames.FONTTYPE_INFO)
...
If UserList(VictimaIndex).flags.Privilegios And PlayerType.User Then
    ...
    If res < 3 Then 'Exito robo
        ...
        If (RandomNumber(1, 50) < 25) And (UserList(LadrOnIndex).Clase = eClass.Ladron) Then
            ...
            Else
                Call WriteConsoleMsg(1, LadrOnIndex, UserList(VictimaIndex).name & " no tiene objetos.", FontTypeNames.FONTTYPE_INFO)
            End If
        Else 'Roba oro
            If UserList(VictimaIndex).Stats.GLD > 0 Then
                ...
                Call WriteConsoleMsg(1, LadrOnIndex, "Le has robado " & N & " monedas de oro a " & UserList(VictimaIndex).name, FontTypeNames.FONTTYPE_INFO)
            ...
            Else
                Call WriteConsoleMsg(1, LadrOnIndex, UserList(VictimaIndex).name & " no tiene oro.", FontTypeNames.FONTTYPE_INFO)
            End If
        End If
    Else
        Call WriteConsoleMsg(1, LadrOnIndex, "¡No has logrado robar nada!", FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, VictimaIndex, "¡" & UserList(LadrOnIndex).name & " ha intentado robarte!", FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, VictimaIndex, "¡" & UserList(LadrOnIndex).name & " es un criminal!", FontTypeNames.FONTTYPE_INFO)
        Call FlushBuffer(VictimaIndex)
    End If

    If Not esRene(LadrOnIndex) And Not esFaccion(LadrOnIndex) Then
...
If flag Then
    ...
    Else
        Call WriteConsoleMsg(1, LadrOnIndex, "Has hurtado " & MiObj.amount & " " & ObjData(MiObj.ObjIndex).name, FontTypeNames.FONTTYPE_INFO)
    End If
Else
...
If RandomNumber(0, 100) < Suerte Then
    If VictimUserIndex <> 0 Then
        ...
        Call WriteConsoleMsg(1, UserIndex, "Has apuñalado a " & UserList(VictimUserIndex).name & " por " & daño, FontTypeNames.FONTTYPE_FIGHT)
        Call WriteConsoleMsg(1, VictimUserIndex, "Te ha apuñalado " & UserList(UserIndex).name & " por " & daño, FontTypeNames.FONTTYPE_FIGHT)
        
        Call FlushBuffer(VictimUserIndex)
    Else
        Npclist(VictimNpcIndex).Stats.MinHP = Npclist(VictimNpcIndex).Stats.MinHP - Int(daño * 2)
        Call WriteConsoleMsg(1, UserIndex, "Has apuñalado la criatura por " & Int(daño * 2), FontTypeNames.FONTTYPE_FIGHT)
        Call SubirSkill(UserIndex, Apuñalar)
        '[Alejo]
        Call CalcularDarExp(UserIndex, VictimNpcIndex, daño * 2)
    End If
Else
    Call WriteConsoleMsg(1, UserIndex, "¡No has logrado apuñalar a tu enemigo!", FontTypeNames.FONTTYPE_FIGHT)
End If
...
If MapInfo(UserList(UserIndex).Pos.Map).Pk = False Then
   Call WriteConsoleMsg(1, UserIndex, "No esta permitido talar en zonas seguras.", FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
...
If res <= 6 Then
    ...
    Call WriteConsoleMsg(1, UserIndex, "¡Has conseguido algo de leña!", FontTypeNames.FONTTYPE_INFO)
...
If res <= 5 Then
    ...
    Call WriteConsoleMsg(1, UserIndex, "¡Has extraido algunos minerales!", FontTypeNames.FONTTYPE_INFO)
    
Else
    '[CDT 17-02-2004]
    If Not UserList(UserIndex).flags.UltimoMensaje = 9 Then
        Call WriteConsoleMsg(1, UserIndex, "¡No has conseguido nada!", FontTypeNames.FONTTYPE_INFO)
...
If TActual - UserList(UserIndex).Counters.IntervaloMeditar < 540 Then
    Exit Sub
Else
    UserList(UserIndex).Counters.IntervaloMeditar = GetTickCount()
End If
...
'If res = 1 Then
    ...
    Call WriteConsoleMsg(1, UserIndex, "¡Has recuperado " & cant & " puntos de mana!", FontTypeNames.FONTTYPE_INFO)
...
'***************************************************
If UserList(UserIndex).Clase <> eClass.Gladiador Then Exit Sub
...
If (res < 20) Then
    If TieneObjetosRobables(VictimaIndex) Then
        ...
        Call WriteConsoleMsg(1, VictimaIndex, "¡" & UserList(UserIndex).name & " es un Bandido!", FontTypeNames.FONTTYPE_INFO)
    Else
        Call WriteConsoleMsg(1, UserIndex, UserList(VictimaIndex).name & " no tiene objetos.", FontTypeNames.FONTTYPE_INFO)
    End If
End If
...
'shermie80 - Maycolito Shoy sho :D
Dim res As Integer
If (RandomNumber(1, 100) <= 25) Then
If res < TieneObjetos(869, 1, UserIndex) Then Exit Sub
    ...
    Call WriteConsoleMsg(1, UserIndex, "Tu golpe ha Paralizo a tu oponente", FontTypeNames.FONTTYPE_FIGHT)
    Call WriteConsoleMsg(1, VictimaIndex, "¡El golpe ha Paralizado!", FontTypeNames.FONTTYPE_INFO)
...
If UserList(UserIndex).Stats.UserSkills(eSkill.artes) <= 10 _
   And UserList(UserIndex).Stats.UserSkills(eSkill.artes) >= -1 Then
                    Suerte = 35
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) <= 20 _
   And UserList(UserIndex).Stats.UserSkills(eSkill.artes) >= 11 Then
                    Suerte = 30
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) <= 30 _
   And UserList(UserIndex).Stats.UserSkills(eSkill.artes) >= 21 Then
                    Suerte = 28
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) <= 40 _
   And UserList(UserIndex).Stats.UserSkills(eSkill.artes) >= 31 Then
                    Suerte = 24
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) <= 50 _
   And UserList(UserIndex).Stats.UserSkills(eSkill.artes) >= 41 Then
                    Suerte = 22
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) <= 60 _
   And UserList(UserIndex).Stats.UserSkills(eSkill.artes) >= 51 Then
                    Suerte = 20
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) <= 70 _
   And UserList(UserIndex).Stats.UserSkills(eSkill.artes) >= 61 Then
                    Suerte = 18
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) <= 80 _
   And UserList(UserIndex).Stats.UserSkills(eSkill.artes) >= 71 Then
                    Suerte = 15
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) <= 90 _
   And UserList(UserIndex).Stats.UserSkills(eSkill.artes) >= 81 Then
                    Suerte = 10
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) < 100 _
   And UserList(UserIndex).Stats.UserSkills(eSkill.artes) >= 91 Then
                    Suerte = 7
ElseIf UserList(UserIndex).Stats.UserSkills(eSkill.artes) = 100 Then
                    Suerte = 5
End If
res = RandomNumber(1, Suerte)
...
If res <= 2 Then
        ...
        Call WriteConsoleMsg(1, UserIndex, "Has logrado desarmar a tu oponente!", FontTypeNames.FONTTYPE_FIGHT)
        If UserList(VictimIndex).Stats.ELV < 20 Then
            Call WriteConsoleMsg(1, VictimIndex, "Tu oponente te ha desarmado!", FontTypeNames.FONTTYPE_FIGHT)
        End If
