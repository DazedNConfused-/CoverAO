            ...
            If MapData(map, X, Y).NpcIndex > 0 Then     'Acciones NPCs
                ...
                If Npclist(tempIndex).Comercia = 1 Then
                    ...
                    If Distancia(Npclist(tempIndex).Pos, .Pos) > 3 Then
                        Call WriteConsoleMsg(1, UserIndex, "Estas demasiado lejos del vendedor.", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                ...
                ElseIf Npclist(tempIndex).NPCtype = eNPCType.Revividor Or Npclist(tempIndex).NPCtype = eNPCType.ResucitadorNewbie Then
                    If Distancia(.Pos, Npclist(tempIndex).Pos) > 10 Then
                        Call WriteConsoleMsg(1, UserIndex, "El sacerdote no puede curarte debido a que estas demasiado lejos.", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                 ...
                 ElseIf Npclist(tempIndex).NPCtype = 18 Then
                    If Distancia(.Pos, Npclist(tempIndex).Pos) > 3 Then
                        Call WriteConsoleMsg(1, UserIndex, "Estas lejos.", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
...
Call WriteShowCorreo(UserIndex)
End Sub
    ...
    If ObjData(MapData(map, X, Y).ObjInfo.ObjIndex).Llave = 0 Then
        If ObjData(MapData(map, X, Y).ObjInfo.ObjIndex).Cerrada = 1 Then
                ...
                If ObjData(MapData(map, X, Y).ObjInfo.ObjIndex).Llave = 0 Then
                    ...
                    Call modSendData.SendToAreaByPos(map, X, Y, PrepareMessageObjectCreate(ObjData(MapData(map, X, Y).ObjInfo.ObjIndex).GrhIndex, X, Y, MapData(map, X, Y).ObjInfo.ObjIndex))
