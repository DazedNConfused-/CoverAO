...
Sub ActStats(ByVal VictimIndex As Integer, ByVal attackerIndex As Integer)
    ...
    With UserList(attackerIndex)
        ...
        Call WriteConsoleMsg(2, attackerIndex, "Has ganado " & DaExp & " puntos de experiencia.", FontTypeNames.FONTTYPE_FIGHT)
...
Sub DoResucitar(ByVal UserIndex As Integer)
    With UserList(UserIndex)
        If Not .flags.Resucitando = True Then Exit Sub
        
        Dim TActual As Long
        TActual = GetTickCount() And &H7FFFFFFF
        If TActual - UserList(UserIndex).Counters.IntervaloRevive < 5000 Then
            Exit Sub
        Else
            DarVida UserIndex
        End If
...
On Error GoTo hayerror
    ...
    If InMapBounds(map, X, Y) Then
        ...
        If LenB(klan) <> 0 Then
            If Not toMap Then
                If UserList(UserIndex).showName Then
                    Call WriteCharacterCreate(sndIndex, UserList(UserIndex).Char.body, UserList(UserIndex).Char.Head, UserList(UserIndex).Char.heading, UserList(UserIndex).Char.CharIndex, X, Y, UserList(UserIndex).Char.WeaponAnim, UserList(UserIndex).Char.ShieldAnim, UserList(UserIndex).Char.FX, 999, UserList(UserIndex).Char.CascoAnim, UserList(UserIndex).name & " <" & klan & ">", bCr, UserTypeColor(UserIndex))
                Else
                    'Hide the name and clan - set privs as normal user
                    Call WriteCharacterCreate(sndIndex, UserList(UserIndex).Char.body, UserList(UserIndex).Char.Head, UserList(UserIndex).Char.heading, UserList(UserIndex).Char.CharIndex, X, Y, UserList(UserIndex).Char.WeaponAnim, UserList(UserIndex).Char.ShieldAnim, UserList(UserIndex).Char.FX, 999, UserList(UserIndex).Char.CascoAnim, vbNullString, bCr, UserTypeColor(UserIndex))
                End If
            Else
        ...
        Else 'if tiene clan
            If Not toMap Then
                If UserList(UserIndex).showName Then
                    Call WriteCharacterCreate(sndIndex, UserList(UserIndex).Char.body, UserList(UserIndex).Char.Head, UserList(UserIndex).Char.heading, UserList(UserIndex).Char.CharIndex, X, Y, UserList(UserIndex).Char.WeaponAnim, UserList(UserIndex).Char.ShieldAnim, UserList(UserIndex).Char.FX, 999, UserList(UserIndex).Char.CascoAnim, UserList(UserIndex).name, bCr, UserTypeColor(UserIndex))
                Else
                    Call WriteCharacterCreate(sndIndex, UserList(UserIndex).Char.body, UserList(UserIndex).Char.Head, UserList(UserIndex).Char.heading, UserList(UserIndex).Char.CharIndex, X, Y, UserList(UserIndex).Char.WeaponAnim, UserList(UserIndex).Char.ShieldAnim, UserList(UserIndex).Char.FX, 999, UserList(UserIndex).Char.CascoAnim, vbNullString, bCr, UserTypeColor(UserIndex))
                End If
            Else
...
On Error GoTo Errhandler
    ...
    With UserList(UserIndex)
        Do While .Stats.Exp >= .Stats.ELU
            ...
            Call WriteConsoleMsg(1, UserIndex, "¡Has subido de nivel!", FontTypeNames.FONTTYPE_INFO)
            ...
            Call WriteConsoleMsg(1, UserIndex, "Servidor>Ya no Seguiras Obteniendo Bonificaciones ya eres Nivel 50.", FontTypeNames.FONTTYPE_INFO)
            Call WriteUpdateUserStats(UserIndex)
            Exit Sub
            End If
...
'TODO : Sacar este IF en la 0.13 y dejar sólo el Else (ToxicWaste)
          ...
          Select Case .Clase
                ...
                Case eClass.Mago
                    AumentoHIT = 1
                    AumentoMANA = 2.7 * .Stats.UserAtributos(eAtributos.Inteligencia)
                ...
                Case eClass.Asesino
                    AumentoHIT = IIf(.Stats.ELV > 35, 1, 3)
                    AumentoMANA = 0.95 * .Stats.UserAtributos(eAtributos.Inteligencia)
                    AumentoSTA = AumentoSTDef
            ...
            If .Stats.ELV = 25 Then
                ...
                If GI > 0 Then
                    If modGuilds.GuildAlignment(GI) = "Legión oscura" Or modGuilds.GuildAlignment(GI) = "Armada Real" Then
                        ...
                        Call WriteConsoleMsg(1, UserIndex, "¡Ya tienes la madurez suficiente como para decidir bajo que estandarte pelearás! Por esta razón, hasta tanto no te enlistes en la Facción bajo la cual tu clan está alineado, estarás excluído del mismo.", FontTypeNames.FONTTYPE_GUILD)
                    End If
                End If
            End If
...
Public Sub SendUserStatsTxt(ByVal sendIndex As Integer, ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        Call WriteConsoleMsg(1, sendIndex, "Estadisticas de: " & .name, FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Nivel: " & .Stats.ELV & "  EXP: " & .Stats.Exp & "/" & .Stats.ELU, FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Salud: " & .Stats.MinHP & "/" & .Stats.MaxHP & "  Mana: " & .Stats.MinMAN & "/" & .Stats.MaxMAN & "  Vitalidad: " & .Stats.MinSta & "/" & .Stats.MaxSta, FontTypeNames.FONTTYPE_INFO)
        ...
        If .Invent.ArmourEqpObjIndex > 0 Then
            If .Invent.EscudoEqpObjIndex > 0 Then
                Call WriteConsoleMsg(1, sendIndex, "(CUERPO) Min Def/Max Def: " & ObjData(.Invent.ArmourEqpObjIndex).MinDef + ObjData(.Invent.EscudoEqpObjIndex).MinDef & "/" & ObjData(.Invent.ArmourEqpObjIndex).MaxDef + ObjData(.Invent.EscudoEqpObjIndex).MaxDef, FontTypeNames.FONTTYPE_INFO)
            Else
                Call WriteConsoleMsg(1, sendIndex, "(CUERPO) Min Def/Max Def: " & ObjData(.Invent.ArmourEqpObjIndex).MinDef & "/" & ObjData(.Invent.ArmourEqpObjIndex).MaxDef, FontTypeNames.FONTTYPE_INFO)
            End If
        Else
            Call WriteConsoleMsg(1, sendIndex, "(CUERPO) Min Def/Max Def: 0", FontTypeNames.FONTTYPE_INFO)
        End If
        
        If .Invent.CascoEqpObjIndex > 0 Then
            Call WriteConsoleMsg(1, sendIndex, "(CABEZA) Min Def/Max Def: " & ObjData(.Invent.CascoEqpObjIndex).MinDef & "/" & ObjData(.Invent.CascoEqpObjIndex).MaxDef, FontTypeNames.FONTTYPE_INFO)
        Else
            Call WriteConsoleMsg(1, sendIndex, "(CABEZA) Min Def/Max Def: 0", FontTypeNames.FONTTYPE_INFO)
        End If
        ...
        If GuildI > 0 Then
            ...
            If UCase$(modGuilds.GuildLeader(GuildI)) = UCase$(.name) Then
                Call WriteConsoleMsg(1, sendIndex, "Status: Lider", FontTypeNames.FONTTYPE_INFO)
            End If
            'guildpts no tienen objeto
        End If
        
        Call WriteConsoleMsg(1, sendIndex, "Oro: " & .Stats.GLD & "  Posicion: " & .Pos.X & "," & .Pos.Y & " en mapa " & .Pos.map, FontTypeNames.FONTTYPE_INFO)
...
'*************************************************
    With UserList(UserIndex)
        Call WriteConsoleMsg(1, sendIndex, "Pj: " & .name, FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "NPCsMuertos: " & .Stats.NPCsMuertos, FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Clase: " & ListaClases(.Clase), FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Pena: " & .Counters.Pena, FontTypeNames.FONTTYPE_INFO)
        
        If .GuildIndex > 0 Then
            Call WriteConsoleMsg(1, sendIndex, "Clan: " & GuildName(.GuildIndex), FontTypeNames.FONTTYPE_INFO)
        End If
    End With
End Sub
...
'*************************************************
    ...
    If FileExist(CharFile) Then
        Call WriteConsoleMsg(1, sendIndex, "Pj: " & charName, FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "CiudadanosMatados: " & GetVar(CharFile, "FACCIONES", "CiudMatados") & " CriminalesMatados: " & GetVar(CharFile, "FACCIONES", "CrimMatados") & " UsuariosMatados: " & GetVar(CharFile, "MUERTES", "UserMuertes"), FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "NPCsMuertos: " & GetVar(CharFile, "MUERTES", "NpcsMuertes"), FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Clase: " & ListaClases(GetVar(CharFile, "INIT", "Clase")), FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Pena: " & GetVar(CharFile, "COUNTERS", "PENA"), FontTypeNames.FONTTYPE_INFO)
        
        If CByte(GetVar(CharFile, "FACCIONES", "EjercitoReal")) = 1 Then
            Call WriteConsoleMsg(1, sendIndex, "Armada Real Desde: " & GetVar(CharFile, "FACCIONES", "FechaIngreso"), FontTypeNames.FONTTYPE_INFO)
            Call WriteConsoleMsg(1, sendIndex, "Ingresó en Nivel: " & CInt(GetVar(CharFile, "FACCIONES", "NivelIngreso")) & " con " & CInt(GetVar(CharFile, "FACCIONES", "MatadosIngreso")) & " Ciudadanos matados.", FontTypeNames.FONTTYPE_INFO)
            Call WriteConsoleMsg(1, sendIndex, "Veces que Ingresó: " & CByte(GetVar(CharFile, "FACCIONES", "Reenlistadas")), FontTypeNames.FONTTYPE_INFO)
        
        ElseIf CByte(GetVar(CharFile, "FACCIONES", "EjercitoCaos")) = 1 Then
            Call WriteConsoleMsg(1, sendIndex, "Legion Oscura Desde: " & GetVar(CharFile, "FACCIONES", "FechaIngreso"), FontTypeNames.FONTTYPE_INFO)
            Call WriteConsoleMsg(1, sendIndex, "Ingresó en Nivel: " & CInt(GetVar(CharFile, "FACCIONES", "NivelIngreso")), FontTypeNames.FONTTYPE_INFO)
            Call WriteConsoleMsg(1, sendIndex, "Veces que Ingresó: " & CByte(GetVar(CharFile, "FACCIONES", "Reenlistadas")), FontTypeNames.FONTTYPE_INFO)
        
        ElseIf CByte(GetVar(CharFile, "FACCIONES", "rExReal")) = 1 Then
            Call WriteConsoleMsg(1, sendIndex, "Fue Armada Real", FontTypeNames.FONTTYPE_INFO)
            Call WriteConsoleMsg(1, sendIndex, "Veces que Ingresó: " & CByte(GetVar(CharFile, "FACCIONES", "Reenlistadas")), FontTypeNames.FONTTYPE_INFO)
        
        ElseIf CByte(GetVar(CharFile, "FACCIONES", "rExCaos")) = 1 Then
            Call WriteConsoleMsg(1, sendIndex, "Fue Legionario", FontTypeNames.FONTTYPE_INFO)
            Call WriteConsoleMsg(1, sendIndex, "Veces que Ingresó: " & CByte(GetVar(CharFile, "FACCIONES", "Reenlistadas")), FontTypeNames.FONTTYPE_INFO)
        End If

        
        Call WriteConsoleMsg(1, sendIndex, "Asesino: " & CLng(GetVar(CharFile, "REP", "Asesino")), FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Noble: " & CLng(GetVar(CharFile, "REP", "Nobles")), FontTypeNames.FONTTYPE_INFO)
        
        If IsNumeric(GetVar(CharFile, "Guild", "GUILDINDEX")) Then
            Call WriteConsoleMsg(1, sendIndex, "Clan: " & modGuilds.GuildName(CInt(GetVar(CharFile, "Guild", "GUILDINDEX"))), FontTypeNames.FONTTYPE_INFO)
        End If
        
        Ban = GetVar(CharFile, "FLAGS", "Ban")
        Call WriteConsoleMsg(1, sendIndex, "Ban: " & Ban, FontTypeNames.FONTTYPE_INFO)
        
        If Ban = "1" Then
            Call WriteConsoleMsg(1, sendIndex, "Ban por: " & GetVar(CharFile, charName, "BannedBy") & " Motivo: " & GetVar(BanDetailPath, charName, "Reason"), FontTypeNames.FONTTYPE_INFO)
        End If
    Else
        Call WriteConsoleMsg(1, sendIndex, "El pj no existe: " & charName, FontTypeNames.FONTTYPE_INFO)
    End If
End Sub
...
On Error Resume Next
    ...
    With UserList(UserIndex)
        Call WriteConsoleMsg(1, sendIndex, .name, FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Tiene " & .Invent.NroItems & " objetos.", FontTypeNames.FONTTYPE_INFO)
        
        For j = 1 To MAX_INVENTORY_SLOTS
            If .Invent.Object(j).ObjIndex > 0 Then
                Call WriteConsoleMsg(1, sendIndex, " Objeto " & j & " " & ObjData(.Invent.Object(j).ObjIndex).name & " Cantidad:" & .Invent.Object(j).amount, FontTypeNames.FONTTYPE_INFO)
            End If
        Next j
    End With
End Sub
...
On Error Resume Next
    ...
    If FileExist(CharFile, vbNormal) Then
        Call WriteConsoleMsg(1, sendIndex, charName, FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, " Tiene " & GetVar(CharFile, "Inventory", "CantidadItems") & " objetos.", FontTypeNames.FONTTYPE_INFO)
        ...
        For j = 1 To MAX_INVENTORY_SLOTS
            ...
            If ObjInd > 0 Then
                Call WriteConsoleMsg(1, sendIndex, " Objeto " & j & " " & ObjData(ObjInd).name & " Cantidad:" & ObjCant, FontTypeNames.FONTTYPE_INFO)
            End If
        Next j
    Else
        Call WriteConsoleMsg(1, sendIndex, "Usuario inexistente: " & charName, FontTypeNames.FONTTYPE_INFO)
    End If
End Sub
...
On Error Resume Next
    ...
    Call WriteConsoleMsg(1, sendIndex, UserList(UserIndex).name, FontTypeNames.FONTTYPE_INFO)
    
    For j = 1 To NUMSKILLS
        Call WriteConsoleMsg(1, sendIndex, SkillsNames(j) & " = " & UserList(UserIndex).Stats.UserSkills(j), FontTypeNames.FONTTYPE_INFO)
    Next j
    
    Call WriteConsoleMsg(1, sendIndex, " SkillLibres:" & UserList(UserIndex).Stats.SkillPts, FontTypeNames.FONTTYPE_INFO)
End Sub
...
Private Function EsMascotaCiudadano(ByVal NpcIndex As Integer, ByVal UserIndex As Integer) As Boolean
    ...
    If Npclist(NpcIndex).MaestroUser > 0 Then
        ...
        If EsMascotaCiudadano Then
            Call WriteConsoleMsg(1, Npclist(NpcIndex).MaestroUser, "¡¡" & UserList(UserIndex).name & " esta atacando tu mascota!!", FontTypeNames.FONTTYPE_INFO)
        End If
    End If
End Function
    ...
    With UserList(UserIndex)
        If .flags.Hambre = 0 And .flags.Sed = 0 Then
            ...
            If RandomNumber(1, Prob) = 7 Then
                ...
                Call WriteConsoleMsg(1, UserIndex, "¡Has mejorado tu skill " & SkillsNames(Skill) & " en un punto!. Ahora tienes " & .Stats.UserSkills(Skill) & " pts.", FontTypeNames.FONTTYPE_INFO)
                
                .Stats.Exp = .Stats.Exp + 50
                If .Stats.Exp > MAXEXP Then .Stats.Exp = MAXEXP
                
                Call WriteConsoleMsg(1, UserIndex, "¡Has ganado 50 puntos de experiencia!", FontTypeNames.FONTTYPE_FIGHT)
...
On Error GoTo ErrorHandler
    ...
    'Si estaba en algun evento sacarlo
    If UserList(UserIndex).flags.EnDuelo = 1 Then
        Call WarpUserChar(UserIndex, UserList(UserIndex).AuxPos.map, UserList(UserIndex).AuxPos.X, UserList(UserIndex).AuxPos.Y, True)
        Call modSendData.SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(1, "Duelos> " & UserList(UserIndex).name & " fue derrotado.", FontTypeNames.FONTTYPE_INFO))
...
Sub ContarMuerte(ByVal muerto As Integer, ByVal Atacante As Integer)
    ...
    With UserList(Atacante)
        ...
        If UserList(muerto).Faccion.ArmadaReal = 1 Then .Faccion.ArmadaMatados = .Faccion.ArmadaMatados + 1
        If UserList(muerto).Faccion.Milicia = 1 Then .Faccion.MilicianosMatados = .Faccion.MilicianosMatados + 1
        If UserList(muerto).Faccion.FuerzasCaos = 1 Then .Faccion.CaosMatados = .Faccion.CaosMatados + 1
    End With
End Sub
...
Sub WarpUserChar(ByVal UserIndex As Integer, ByVal map As Integer, ByVal X As Integer, ByVal Y As Integer, ByVal FX As Boolean)
    ...
    With UserList(UserIndex)
        ...
        If OldMap <> map Then
            Call WriteChangeMap(UserIndex, map, MapInfo(.Pos.map).MapVersion, MapInfo(.Pos.map).name)
...
'***************************************************
    ...
    If UserList(UserIndex).flags.UserLogged And Not UserList(UserIndex).Counters.Saliendo Then
        ...
        Call WriteConsoleMsg(1, UserIndex, "Cerrando...Se cerrará el juego en " & UserList(UserIndex).Counters.Salir & " segundos...", FontTypeNames.FONTTYPE_INFO)
    End If
End Sub
...
'***************************************************
    If UserList(UserIndex).Counters.Saliendo Then
        ...
        If UserList(UserIndex).ConnIDValida Then
            ...
            Call WriteConsoleMsg(1, UserIndex, "/salir cancelado.", FontTypeNames.FONTTYPE_WARNING)
        Else
            'Simply reset
            UserList(UserIndex).Counters.Salir = IIf((UserList(UserIndex).flags.Privilegios And PlayerType.User) And MapInfo(UserList(UserIndex).Pos.map).Pk, IntervaloCerrarConexion, 0)
        End If
...
Sub SendUserStatsTxtOFF(ByVal sendIndex As Integer, ByVal Nombre As String)
    If FileExist(CharPath & Nombre & ".chr", vbArchive) = False Then
        Call WriteConsoleMsg(1, sendIndex, "Pj Inexistente", FontTypeNames.FONTTYPE_INFO)
    Else
        Call WriteConsoleMsg(1, sendIndex, "Estadisticas de: " & Nombre, FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Nivel: " & GetVar(CharPath & Nombre & ".chr", "stats", "elv") & "  EXP: " & GetVar(CharPath & Nombre & ".chr", "stats", "Exp") & "/" & GetVar(CharPath & Nombre & ".chr", "stats", "elu"), FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Vitalidad: " & GetVar(CharPath & Nombre & ".chr", "stats", "minsta") & "/" & GetVar(CharPath & Nombre & ".chr", "stats", "maxSta"), FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, "Salud: " & GetVar(CharPath & Nombre & ".chr", "stats", "MinHP") & "/" & GetVar(CharPath & Nombre & ".chr", "Stats", "MaxHP") & "  Mana: " & GetVar(CharPath & Nombre & ".chr", "Stats", "MinMAN") & "/" & GetVar(CharPath & Nombre & ".chr", "Stats", "MaxMAN"), FontTypeNames.FONTTYPE_INFO)
        
        Call WriteConsoleMsg(1, sendIndex, "Menor Golpe/Mayor Golpe: " & GetVar(CharPath & Nombre & ".chr", "stats", "MaxHIT"), FontTypeNames.FONTTYPE_INFO)
        
        Call WriteConsoleMsg(1, sendIndex, "Oro: " & GetVar(CharPath & Nombre & ".chr", "stats", "GLD"), FontTypeNames.FONTTYPE_INFO)
...
#If ConUpTime Then
        ...
        Call WriteConsoleMsg(1, sendIndex, "Tiempo Logeado: " & TempStr, FontTypeNames.FONTTYPE_INFO)
#End If
...
On Error Resume Next
    ...
    If FileExist(CharFile, vbNormal) Then
        Call WriteConsoleMsg(1, sendIndex, charName, FontTypeNames.FONTTYPE_INFO)
        Call WriteConsoleMsg(1, sendIndex, " Tiene " & GetVar(CharFile, "STATS", "BANCO") & " en el banco.", FontTypeNames.FONTTYPE_INFO)
    Else
        Call WriteConsoleMsg(1, sendIndex, "Usuario inexistente: " & charName, FontTypeNames.FONTTYPE_INFO)
    End If
End Sub
    ...
    Else
        ...
        If LenB(Chat) <> 0 Then
            ...
            If .flags.Privilegios And PlayerType.User Then
                Call SendData(SendTarget.toMap, UserIndex, PrepareMessageChatOverHead(Chat, .Char.CharIndex, vbRed))
            Else
                Call SendData(SendTarget.toMap, UserIndex, PrepareMessageChatOverHead(Chat, .Char.CharIndex, CHAT_COLOR_GM_YELL))
            End If
        End If
    End If
End With
...
Function UserTypeColor(ByVal UserIndex As Integer) As Byte
    ...
    End If
End Function
Function UserTypeColorAcc(ByVal NameUser As String) As Byte
    Dim Renegado As Byte, Armada As Byte, Caos As Byte, Repu As Byte, Impe As Byte, Mili As Byte
    Dim FileS As String
    FileS = App.Path & "\Charfile\" & UCase$(NameUser) & ".chr"
    
    If Not FileExist(FileS, vbNormal) Then
        UserTypeColorAcc = 1
        Exit Function
    End If
    
    Armada = CByte(GetVar(FileS, "FACCIONES", "EjercitoReal"))
    Caos = CByte(GetVar(FileS, "FACCIONES", "EjercitoCaos"))
    Mili = CByte(GetVar(FileS, "FACCIONES", "EjercitoMili"))
    Impe = CByte(GetVar(FileS, "FACCIONES", "Ciudadano"))
    Repu = CByte(GetVar(FileS, "FACCIONES", "Republicano"))
    Renegado = CByte(GetVar(FileS, "FACCIONES", "Renegado"))
