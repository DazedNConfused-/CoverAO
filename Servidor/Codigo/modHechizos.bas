...
If Hechizos(Spell).SubeHP = 1 Then
    ...
    Call WriteConsoleMsg(1, UserIndex, Npclist(NpcIndex).name & " te ha quitado " & daño & " puntos de vida.", FontTypeNames.FONTTYPE_FIGHT)
...
If Hechizos(Spell).Paraliza = 1 Or Hechizos(Spell).Inmoviliza = 1 Then
    If UserList(UserIndex).flags.Paralizado = 0 Then
        ...
        If UserList(UserIndex).Invent.AnilloEqpObjIndex = SUPERANILLO Then
            Call WriteConsoleMsg(1, UserIndex, " Tu anillo rechaza los efectos del hechizo.", FontTypeNames.FONTTYPE_FIGHT)
            Exit Sub
        End If
...
On Error Resume Next
    If TieneObjetos(755, 1, UserIndex) Then Exit Sub
...
Dim DruidManaBonus As Single
    ...
    If UserList(UserIndex).flags.muerto Then
        Call WriteConsoleMsg(1, UserIndex, "No podes lanzar hechizos porque estas muerto.", FontTypeNames.FONTTYPE_INFO)
        PuedeLanzar = False
        Exit Function
    End If
        
If Hechizos(HechizoIndex).NeedAnillo > 0 Then
            If UserList(UserIndex).Invent.AnilloEqpObjIndex > 1 Then
                If ObjData(UserList(UserIndex).Invent.AnilloEqpObjIndex).AnilloPower < Hechizos(HechizoIndex).NeedAnillo Then
                    'Call WriteConsoleMsg(1, UserIndex, "No posees un Anillo Espectral para lanzar el conjuro.", FontTypeNames.FONTTYPE_INFO)
                    PuedeLanzar = True
                    Exit Function
                End If
            Else
                Call WriteConsoleMsg(1, UserIndex, "Para lanzar Ese hechizo Debes Tener un Anillo Espectral o un Anillo de las Penumbras.", FontTypeNames.FONTTYPE_INFO)
                PuedeLanzar = False
                Exit Function
            End If
    ...
    'If it is a summoning spell and we are druids, having a magic flute equipped then we will need less mana
        If UserList(UserIndex).Clase = eClass.Druida And UserList(UserIndex).Invent.AnilloEqpObjIndex = FLAUTAMAGICA Then
            DruidManaBonus = 0.7
        Else
            DruidManaBonus = 1
        End If
...
If b Then
    ...
    If UserList(UserIndex).Clase = eClass.Druida And UserList(UserIndex).Invent.AnilloEqpObjIndex = FLAUTAMAGICA And Hechizos(uh).Mimetiza = 1 Then
...
If Hechizos(H).Invisibilidad = 1 Then
    ...
    If UserList(tU).flags.muerto = 1 Then
        Call WriteConsoleMsg(1, UserIndex, "¡Está muerto!", FontTypeNames.FONTTYPE_INFO)
        b = False
        Exit Sub
    End If
    ...
    If UserList(tU).Counters.Saliendo Then
        If UserIndex <> tU Then
            Call WriteConsoleMsg(1, UserIndex, "¡El hechizo no tiene efecto!", FontTypeNames.FONTTYPE_INFO)
            b = False
            Exit Sub
        Else
    ...
    If (TriggerZonaPelea(UserIndex, tU) <> TRIGGER6_PERMITE) Then
        If criminal(tU) And Not criminal(UserIndex) Then
            If esArmada(UserIndex) Then
                Call WriteConsoleMsg(1, UserIndex, "Los miembros de la armada real no pueden ayudar a los criminales", FontTypeNames.FONTTYPE_INFO)
                b = False
                Exit Sub
            End If
            If UserList(UserIndex).flags.Seguro Then
                Call WriteConsoleMsg(1, UserIndex, "Para ayudar criminales debes sacarte el seguro ya que te volverás criminal como ellos", FontTypeNames.FONTTYPE_INFO)
                b = False
                Exit Sub
            Else
...
If Hechizos(H).Envenena = 1 Then
    If UserIndex = tU Then
        Call WriteConsoleMsg(1, UserIndex, "No puedes atacarte a vos mismo.", FontTypeNames.FONTTYPE_FIGHT)
        Exit Sub
    End If
...
If Hechizos(H).CuraVeneno = 1 Then
    ...
    If (TriggerZonaPelea(UserIndex, tU) <> TRIGGER6_PERMITE) Then
        If criminal(tU) And Not criminal(UserIndex) Then
            If esArmada(UserIndex) Then
                Call WriteConsoleMsg(1, UserIndex, "Los Armadas no pueden ayudar a los Criminales", FontTypeNames.FONTTYPE_INFO)
                b = False
                Exit Sub
            End If
            If UserList(UserIndex).flags.Seguro Then
    ...
    End If
     ...
     If UserList(tU).flags.Paralizado = 0 Then
            ...
            If UserList(tU).Invent.AnilloEqpObjIndex = SUPERANILLO Then
                Call WriteConsoleMsg(1, tU, " Tu anillo rechaza los efectos del hechizo.", FontTypeNames.FONTTYPE_FIGHT)
                Call WriteConsoleMsg(1, UserIndex, " ¡El hechizo no tiene efecto!", FontTypeNames.FONTTYPE_FIGHT)
                Call FlushBuffer(tU)
                Exit Sub
            End If
...
If Hechizos(H).Revivir = 1 Then
    If UserList(tU).flags.muerto = 1 Then
        ...
        If UserList(tU).flags.SeguroResu Then
            Call WriteConsoleMsg(1, UserIndex, "¡El espíritu no tiene intenciones de regresar al mundo de los vivos!", FontTypeNames.FONTTYPE_INFO)
            b = False
            Exit Sub
        End If
        ...
        If UserList(UserIndex).Stats.MaxSta <> UserList(UserIndex).Stats.MinSta Then
            Call WriteConsoleMsg(1, UserIndex, "No puedes resucitar si no tienes tu barra de energía llena.", FontTypeNames.FONTTYPE_INFO)
            b = False
            Exit Sub
        End If
        
        'revisamos si necesita vara
        If UserList(UserIndex).Clase = eClass.Mago Then
            If UserList(UserIndex).Invent.WeaponEqpObjIndex > 0 Then
                If ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).StaffPower < Hechizos(H).NeedStaff Then
                    Call WriteConsoleMsg(1, UserIndex, "Necesitas un mejor báculo para este hechizo", FontTypeNames.FONTTYPE_INFO)
                    b = False
                    Exit Sub
                End If
            End If
        ElseIf UserList(UserIndex).Clase = eClass.Bardo Then
            If UserList(UserIndex).Invent.AnilloEqpObjIndex <> LAUDMAGICO Then
                Call WriteConsoleMsg(1, UserIndex, "Necesitas un instrumento mágico para devolver la vida", FontTypeNames.FONTTYPE_INFO)
                b = False
                Exit Sub
            End If
        ElseIf UserList(UserIndex).Clase = eClass.Druida Then
        ...
        If (UserList(UserIndex).Stats.MinHP <= 0) Then
            Call UserDie(UserIndex)
            Call WriteConsoleMsg(1, UserIndex, "El esfuerzo de resucitar fue demasiado grande", FontTypeNames.FONTTYPE_INFO)
            b = False
        Else
            Call WriteConsoleMsg(1, UserIndex, "El esfuerzo de resucitar te ha debilitado", FontTypeNames.FONTTYPE_INFO)
            b = True
        End If
    ...
    '    If UserList(UserIndex).Clase = eClass.Mago Then
            If UserList(UserIndex).Invent.WeaponEqpObjIndex > 0 Then
                If ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).StaffDamageBonus > 0 Then
                    daño = daño + ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).StaffDamageBonus ')) ' / 100
                Else
                    daño = daño - (daño * 0.1)
                End If
            Else
...
If UserList(tempChr).flags.muerto Then
    Call WriteConsoleMsg(1, UserIndex, "No podés lanzar ese hechizo a un muerto.", FontTypeNames.FONTTYPE_INFO)
    Exit Sub
End If
...
If Hechizos(H).SubeHam = 1 Then
    ...
    If UserIndex <> tempChr Then
        Call WriteConsoleMsg(1, UserIndex, "Le has restaurado " & daño & " puntos de hambre a " & UserList(tempChr).name, FontTypeNames.FONTTYPE_FIGHT)
        Call WriteConsoleMsg(1, tempChr, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de hambre.", FontTypeNames.FONTTYPE_FIGHT)
    Else
        Call WriteConsoleMsg(1, UserIndex, "Te has restaurado " & daño & " puntos de hambre.", FontTypeNames.FONTTYPE_FIGHT)
    End If
...
ElseIf Hechizos(H).SubeHam = 2 Then
    ...
    If UserIndex <> tempChr Then
        Call WriteConsoleMsg(1, UserIndex, "Le has quitado " & daño & " puntos de hambre a " & UserList(tempChr).name, FontTypeNames.FONTTYPE_FIGHT)
        Call WriteConsoleMsg(1, tempChr, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de hambre.", FontTypeNames.FONTTYPE_FIGHT)
    Else
        Call WriteConsoleMsg(1, UserIndex, "Te has quitado " & daño & " puntos de hambre.", FontTypeNames.FONTTYPE_FIGHT)
    End If
...
If Hechizos(H).SubeSed = 1 Then
    ...
    If UserIndex <> tempChr Then
      Call WriteConsoleMsg(1, UserIndex, "Le has restaurado " & daño & " puntos de sed a " & UserList(tempChr).name, FontTypeNames.FONTTYPE_FIGHT)
      Call WriteConsoleMsg(1, tempChr, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de sed.", FontTypeNames.FONTTYPE_FIGHT)
    Else
      Call WriteConsoleMsg(1, UserIndex, "Te has restaurado " & daño & " puntos de sed.", FontTypeNames.FONTTYPE_FIGHT)
    End If
...
ElseIf Hechizos(H).SubeSed = 2 Then
    ...
    If UserIndex <> tempChr Then
        Call WriteConsoleMsg(1, UserIndex, "Le has quitado " & daño & " puntos de sed a " & UserList(tempChr).name, FontTypeNames.FONTTYPE_FIGHT)
        Call WriteConsoleMsg(1, tempChr, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de sed.", FontTypeNames.FONTTYPE_FIGHT)
    Else
        Call WriteConsoleMsg(1, UserIndex, "Te has quitado " & daño & " puntos de sed.", FontTypeNames.FONTTYPE_FIGHT)
    End If
...
If Hechizos(H).SubeHP = 1 Then
    ...
    If UserIndex <> tempChr Then
        Call WriteConsoleMsg(1, UserIndex, "Le has restaurado " & daño & " puntos de vida a " & UserList(tempChr).name, FontTypeNames.FONTTYPE_FIGHT)
        Call WriteConsoleMsg(1, tempChr, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de vida.", FontTypeNames.FONTTYPE_FIGHT)
    Else
        Call WriteConsoleMsg(1, UserIndex, "Te has restaurado " & daño & " puntos de vida.", FontTypeNames.FONTTYPE_FIGHT)
    End If
...
ElseIf Hechizos(H).SubeHP = 2 Then
    ...
    If (UserList(tempChr).Invent.WeaponEqpObjIndex > 0) Then
        daño = (daño + ObjData(UserList(tempChr).Invent.WeaponEqpObjIndex).DañoMagico)
    End If
    ...
    Call WriteConsoleMsg(1, UserIndex, "Le has quitado " & daño & " puntos de vida a " & UserList(tempChr).name, FontTypeNames.FONTTYPE_FIGHT)
    Call WriteConsoleMsg(1, tempChr, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de vida.", FontTypeNames.FONTTYPE_FIGHT)
...
If Hechizos(H).SubeMana = 1 Then
    ...
    If UserIndex <> tempChr Then
        Call WriteConsoleMsg(1, UserIndex, "Le has restaurado " & daño & " puntos de mana a " & UserList(tempChr).name, FontTypeNames.FONTTYPE_FIGHT)
        Call WriteConsoleMsg(1, tempChr, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de mana.", FontTypeNames.FONTTYPE_FIGHT)
    Else
        Call WriteConsoleMsg(1, UserIndex, "Te has restaurado " & daño & " puntos de mana.", FontTypeNames.FONTTYPE_FIGHT)
    End If
...
ElseIf Hechizos(H).SubeMana = 2 Then
    ...
    If UserIndex <> tempChr Then
        Call WriteConsoleMsg(1, UserIndex, "Le has quitado " & daño & " puntos de mana a " & UserList(tempChr).name, FontTypeNames.FONTTYPE_FIGHT)
        Call WriteConsoleMsg(1, tempChr, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de mana.", FontTypeNames.FONTTYPE_FIGHT)
    Else
        Call WriteConsoleMsg(1, UserIndex, "Te has quitado " & daño & " puntos de mana.", FontTypeNames.FONTTYPE_FIGHT)
    End If
...
If Hechizos(H).SubeSta = 1 Then
    ...
    If UserIndex <> tempChr Then
        Call WriteConsoleMsg(1, UserIndex, "Le has restaurado " & daño & " puntos de vitalidad a " & UserList(tempChr).name, FontTypeNames.FONTTYPE_FIGHT)
        Call WriteConsoleMsg(1, tempChr, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de vitalidad.", FontTypeNames.FONTTYPE_FIGHT)
    Else
        Call WriteConsoleMsg(1, UserIndex, "Te has restaurado " & daño & " puntos de vitalidad.", FontTypeNames.FONTTYPE_FIGHT)
    End If
    b = True
ElseIf Hechizos(H).SubeSta = 2 Then
    ...
    If UserIndex <> tempChr Then
        Call WriteConsoleMsg(1, UserIndex, "Le has quitado " & daño & " puntos de vitalidad a " & UserList(tempChr).name, FontTypeNames.FONTTYPE_FIGHT)
        Call WriteConsoleMsg(1, tempChr, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de vitalidad.", FontTypeNames.FONTTYPE_FIGHT)
    Else
        Call WriteConsoleMsg(1, UserIndex, "Te has quitado " & daño & " puntos de vitalidad.", FontTypeNames.FONTTYPE_FIGHT)
    End If
